...
export default z.object({
  ...
  rgbw_color: z.tuple([
    ...
    z.number().min(0).max(255),
    z.number().min(0).max(255)
  ]).optional().describe("RGBW color values (0-255)"),
  ...
  rgbww_color: z.tuple([
    ...
    z.number().min(0).max(255),
    z.number().min(0).max(255)
  ]).optional().describe("RGBWW color values (0-255)"),
  ...
  white: z.boolean()
    .default(false)
    .describe("Set the light to white mode"),

  flash: z.enum(['short', 'long'])
    .optional()
