...
This MCP server provides integration with Logz.io, allowing LLMs to search, analyze, and sample log data from Logz.io instances. The server enables natural language interactions with log data for deeper insights and pattern discovery.

## Features

- **Search Tools**: Find specific log entries with powerful query capabilities
...
- **Sampling Tools**: Extract representative samples from large log volumes

## Sampling Features

The server includes two core sampling tools:

1. **sample-logs**: Extract a random sample of logs with optional filtering

   - Control sample size and time range
   - Specify seed values for reproducible samples
   - Filter by query strings
   - Ensure diversity with distinct field sampling

2. **stratified-sample**: Create balanced samples across categories
   - Sample evenly from different dimensions (services, levels, etc.)
   - Ensure representative coverage across your log data
   - Control samples per category for deeper investigation
   - Filter with additional query constraints
