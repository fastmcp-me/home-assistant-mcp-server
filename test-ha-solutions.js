...
async function callHassService(domain, service, data) {
  ...
  try {
    const response = await fetch(`${HASS_URL}/api/services/${domain}/${service}`, {
      method: 'POST',
      headers: {
...
async function testSolution2() {
  ...
  for (const call of serviceFix2) {
    ...
    const result = await callHassService(call.domain, call.service, call.service_data);
    results.push(result);
  }
...
async function runTests() {
  try {
    ...
    console.log('Solution 1 (entity_id in service_data):', result1.success ? 'SUCCESS' : 'FAILED');

    const solution2Success = result2.every(r => r.success);
    console.log('Solution 2 (individual calls):', solution2Success ? 'SUCCESS' : 'FAILED');

    console.log('Solution 3 (brightness instead of brightness_pct):', result3.success ? 'SUCCESS' : 'FAILED');
    console.log('Solution 4 (target and service_data structure):', result4.success ? 'SUCCESS' : 'FAILED');
    ...
    if (result1.success) {
      ...
      console.log('- None of the solutions worked. Manual investigation required.');
    }
  } catch (error) {
